...
We can easily notice that the sum of all intervals in a scale sums to 12. In combinatorics, this is a combination with replacement problem. To count all the possible arrangements, we need to use a technique called stars and bars.
...
How many scales are there if we treat these modes as the same? It now becomes a circular permutation with repetition problem. This is not so simple as it may seem. To solve this, it would require Burnside's theorem to calculate the 'orbit' of each sequence.
