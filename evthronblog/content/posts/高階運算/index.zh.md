...
lastmod: 2026-03-24T12:55:24+08:00
image: 
categories: posts
tags: ['maths']
math: true
...
增長率居然能會隨着數值連貫地上升，實在是很難想象。例如在 $x = 2.5$ 的時候，$e^x$ 的增長率就是 $e^{2.5}$。整數的情況很容易理解：2 次方是自乘兩次，3次方是自乘三次。不過小數要怎麽理解呢？2.5 次方就是自乘 2.5 次嗎？

「乘以 1.5」的意義倒不難理解。乘以 1.5 是加上一半，只是簡單的除法。雖然 1.5 次方可以同樣理解為「再乘上一半」，但因為這涉及了平方根的概念，不像除法一樣直觀，所以不容易懂。

指數已經這麼複雜了，更高階的運算呢？乘法是連續的加法，指數運算是連續的乘法，那麼，連續的指數運算，也就是四階運算（tetration）會怎麼樣？
...
然而，運算階數的提升讓運算規則變得越來越稀少，交換律、分配律、結合律都消失了，只剩下遞歸的定義，找不到自然的連續化方式。沒有連續性，就不用談什麼微積分。
